...
class Condition():  
    def mage(self,se):  
        ...
        if se=='50':
            return mPercent[7]+mPercent[8]+mPercent[9]+mPercent[10]+mPercent[11]+mPercent[12]+mPercent[13]+mPercent[14]
        if se=='상관없음':
            return sum(mPercent)
    def wmage(self,se):
    ...
    def mheight(self,se):
        ...
        if se==0:
            return 100
        if 0<se<=155:
            return sum(mHeight[1:6])/mHeight[0]
        if 155<se<=165:
            return sum(mHeight[6:8])/mHeight[0]
        if 165<se<=175:
            return sum(mHeight[8:10])/mHeight[0]
        if 176<se<=185:
            return sum(mHeight[10:12])/mHeight[0]
        if 185<se:
            return sum(mHeight[12:16])/mHeight[0]
    def wheight(self,se):
        se=int(se)
    ...
    def result(self,gen,a,b,c,d,e,f):
        if gen=='여자':
            return float(dm.mage(a)+dm.mheight(b)+dm.sal(c)+dm.exc(d)+dm.smoke(e)+dm.alc(f))/6
        if gen=='남자':
            return float(dm.wmage(a)+dm.wheight(b)+dm.sal(c)+dm.exc(d)+dm.smoke(e)+dm.alc(f))/6
dm=Condition()
...
class WindowClass(QMainWindow, form_class) :
    ...
    def __init__(self) :
        ...
        self.push4.clicked.connect(self.clearTextFunction)
    ...
    def appendTextFunction(self) :
        ...
        print(keyT)
        print(monB)
        print(self.groupboxRadFunction())
        print(self.exeFunction())
        print(self.smoFunction())
        print(self.driFunction())
        
        resultP=dm.result(self.groupboxRadFunction(),ageB,keyT,monB,self.exeFunction(),self.smoFunction(),
               self.driFunction())
        self.textbrow_Test.append(str(resultP))
